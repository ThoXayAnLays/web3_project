    ...
    const connectWallet = useCallback(async () => {
        if (window.ethereum) {
            try {
                ...
                if (
                    chainId !== parseInt(import.meta.env.VITE_TESTNET_CHAIN_ID)
                ) {
                ...
                const tokenA = new web3Instance.eth.Contract(
                    TokenAABI.abi,
                    contractAddresses.TokenA
                );
                const nftB = new web3Instance.eth.Contract(
                    NFTBABI.abi,
                    contractAddresses.NFTB
                );
                const staking = new web3Instance.eth.Contract(
                    StakingABI.abi,
                ...
                const tokenABalance = await tokenA.methods
                    .balanceOf(accounts[0])
                    .call();
                setTokenABalance(
                    web3Instance.utils.fromWei(tokenABalance, "ether")
                );

                const nftBBalance = await nftB.methods
                    .balanceOf(accounts[0])
                    .call();
                setNFTBBalance(nftBBalance);

                const baseAPR = await staking.methods.baseAPR().call();
                setBaseAPR(web3Instance.utils.fromWei(baseAPR, "ether"));
